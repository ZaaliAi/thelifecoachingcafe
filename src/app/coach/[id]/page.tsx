...
import { Briefcase, MapPin, MessageSquare, Star, Link as LinkIcon, CheckCircle2, BookOpen, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 space-y-12">
        <Button variant="outline" asChild className="mb-4">
          <Link href="/find-a-coach"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Coaches</Link>
        </Button>
      {/* Coach Header */}
      <section className="flex flex-col md:flex-row items-center md:items-start gap-8 p-6 bg-card rounded-lg shadow-xl">
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <Button asChild size="lg" className="mt-6 bg-primary hover:bg-primary/90 text-primary-foreground w-full sm:w-auto">
      ...
      <Card>
        ...
        <CardContent className="prose dark:prose-invert max-w-none text-foreground/90">
          <p>{coach.bio}</p>
          {/* Add more descriptive bio content if available */}
        </CardContent>
      </Card>
      ...
      {(coach.certifications?.length || coach.socialLinks?.length) && (
        <Card>
          <CardHeader>
