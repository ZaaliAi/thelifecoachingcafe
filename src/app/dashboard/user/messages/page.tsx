    ...
    if (user && user.id) {
      ...
      try {
        ...
        for (const msg of fetchedMessages) {
          ...
          let otherPartyAvatar = msg.otherPartyAvatar || undefined; // Start with what's on the message

          // Attempt to enrich with profile data if not already present
          if (!otherPartyName || !otherPartyAvatar) {
            const coachProfile = await getCoachById(otherPartyId); // Assuming users primarily message coaches
            if (coachProfile) {
              otherPartyName = coachProfile.name || otherPartyName || 'Unknown Coach';
              otherPartyAvatar = coachProfile.profileImageUrl || otherPartyAvatar;
            } else {
              // Fallback for non-coach or if coach not found
              ...
              if (userProfile) {
                  ...
                  otherPartyName = otherPartyName || 'Unknown Contact';
              }
            }
          }
            ...
            >
              <CardContent className="p-4 flex items-start space-x-4">
                ...
                <div className="flex flex-col items-end space-y-1">
                    ...
                    <Button variant="outline" size="sm" /* onClick={() => router.push(`/dashboard/user/messages/${otherPartyId}`)} */ >View Thread</Button>
                </div>
              </CardContent>
            </Card>
          ))}
